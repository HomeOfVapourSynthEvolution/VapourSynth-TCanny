...
void outputGB(const float * _srcp, uint16_t * dstp, const int width, const int height, const int srcStride, const int dstStride, const uint16_t peak, const float offset) noexcept {
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x += 8) {
            const Vec8i srcp_8i = truncate_to_int(Vec8f().load_a(_srcp + x) + 0.5f);
...
void discretizeGM(const float * _srcp, uint16_t * dstp, const int width, const int height, const int srcStride, const int dstStride,
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x += 8) {
            const Vec8f srcp_8f = Vec8f().load_a(_srcp + x);
...
template<typename T>
void filter_avx(const VSFrameRef * src, VSFrameRef * dst, const TCannyData * const VS_RESTRICT d, const VSAPI * vsapi) noexcept {
...
template void filter_avx<uint8_t>(const VSFrameRef *, VSFrameRef *, const TCannyData * const VS_RESTRICT, const VSAPI *) noexcept;
template void filter_avx<uint16_t>(const VSFrameRef *, VSFrameRef *, const TCannyData * const VS_RESTRICT, const VSAPI *) noexcept;
template void filter_avx<float>(const VSFrameRef *, VSFrameRef *, const TCannyData * const VS_RESTRICT, const VSAPI *) noexcept;
#endif
